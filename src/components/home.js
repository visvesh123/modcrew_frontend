...
function Home() {
  ...
  useEffect(() => {
    if (selectedCategory === "all") {
      axios
        .get(
          `https://modcrew-dev.herokuapp.com/api/v1/products?limit=${limit}&page=${pageno}`
        )
        .then((response) => {
      ...
      axios
        .get(
          `https://modcrew-dev.herokuapp.com/api/v1/products?category=${selectedCategory}&limit=${limit}&page=${pageno}`
        )
        .then((response) => {
          setSpecificCategoryData(response.data.data);
        });
  ...
  function handleSubCategoryClick(category) {
    ...
    axios
      .get(
        `https://modcrew-dev.herokuapp.com/api/v1/products?category=${category}`
      )
      .then((response) => {
        setCategoryData(response.data.data);
  ...
  return (
    <div>
      ...
      <div className="third-navbar">
        <span 
          onClick={() => {
            handleSubCategoryClick("active-wear");
          }}
          className="third-navbar-spans"
        >
          <img src={activewearActive} style={{marginRight : 6}} />
          <a href="#category-section">Active Wear</a>
        </span>
        <span className="vertical"></span>
        <span
          onClick={() => {
            handleSubCategoryClick("top-wear");
          }}
          className="third-navbar-spans"
        >
          <img src={topwearActive} /> <a href="#category-section">Top Wear</a>
        </span>
        <span className="vertical"></span>
        <span
          onClick={() => {
            handleSubCategoryClick("bottom-wear");
          }}
          className="third-navbar-spans"
        >
          <img src={bottomwearActive} />{" "}
          <a href="#category-section">Bottom Wear</a>
        </span>
        <span className="vertical"></span>
        <span
          onClick={() => {
            handleSubCategoryClick("accessories");
          }}
          className="third-navbar-spans"
        >
          <img src={accessoriesActive} />{" "}
          <a href="#category-section">Accessories</a>
        </span>
        <span className="vertical"></span>
        <span
          onClick={() => {
            handleSubCategoryClick("collectables");
          }}
          className="third-navbar-spans"
        >
          <img src={collectablesActive} />{" "}
          <a href="#category-section">Collectibles</a>
        </span>
      </div>
      <Carousel id={1} img1={cover} img2={coupon1} img3={coupon2} />
      ...
      <div className="row">
        <div className="col-6 content">
          ...
          <button
            className="coupon-btns"
            onMouseOver={() => mouseOverEffectCoupon(1)}
            onMouseOut={() => mouseOutEffectCoupon(1)}
          >
            <span className="use-coupon">Use Coupon </span>
            <br />
            <span className="coupon-code">L657YUOP</span>
          </button>
          <button
            onMouseOver={() => {
              handleOverHover(1);
          ...
          >
            <IoMdCopy size={25} style={{ color: "#EE0405" }} />
          </button>
        </div>
        <div className="col-6 content">
          ...
          <button
            className="coupon-btns"
            onMouseOver={() => mouseOverEffectCoupon(2)}
            onMouseOut={() => mouseOutEffectCoupon(2)}
          >
            <span className="use-coupon">Use Coupon </span>
            <br />
            <span className="coupon-code">L657YUOP</span>
      ...
      <div className="row landing-buttons-tray">
        ...
        <button
          ...
          className={
            typeOfProducts === 1
              ? "landing-page-button landing-page-red col"
              : "landing-page-button col"
          }
        >
          New Arrivals{" "}
        </button>
      ...
      <div id="category-section" class="dropdown category-dropdown-trnsprnt">
        <button
          class="btn btn-secondary dropdown-toggle select-categories-btn"
          type="button"
        ...
        <ul
          class="dropdown-menu list-categories"
